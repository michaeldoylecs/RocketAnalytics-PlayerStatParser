...
Player create_player(vector<Property> player_properties) {
	Player player;
	player.name = player_properties.at(0).get_value_as_string();
	player.platform = player_properties.at(1).get_value_as_string();
	player.onlineID = player_properties.at(2).get_value_as_string();
	player.team = stoi(player_properties.at(3).get_value_as_string());
	player.score = stoi(player_properties.at(4).get_value_as_string());
	player.goals = stoi(player_properties.at(5).get_value_as_string());
	player.assists = stoi(player_properties.at(6).get_value_as_string());
	player.saves = stoi(player_properties.at(7).get_value_as_string());
	player.shots = stoi(player_properties.at(8).get_value_as_string());
	player.games = 1;
	return player;
}
...
int main() {
	...
	for (filesystem::path path : filesystem::directory_iterator(replay_dir)) {
		...
		for (auto player_info : player_stats) {
			Player player = create_player(player_info);
