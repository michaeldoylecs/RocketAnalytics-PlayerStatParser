...
int main() {
	...
	if (!filesystem::is_directory(replay_dir)) {
		cerr << "\nInvalid replay directory path! Program Terminating!"<<
			endl;
		exit(EXIT_FAILURE);
	}
		...
		for (Property prop : replay_properties) {
			if (prop.get_name() == "PlayerStats") {
				...
				for (vector<vector<Property>>::iterator player =
						...
						player++) {
					Player temp_player;
					temp_player.name = player->at(0)
						.get_value_as_string();
					temp_player.platform = player->at(1)
						.get_value_as_string();
					temp_player.onlineID= player->at(2)
						.get_value_as_string();
					temp_player.team = stoi(player->at(3)
						.get_value_as_string());
					temp_player.score = stoi(player->at(4)
						.get_value_as_string());
					temp_player.goals = stoi(player->at(5)
						.get_value_as_string());
					temp_player.assists = stoi(player->at(6)
						.get_value_as_string());
					temp_player.saves = stoi(player->at(7)
						.get_value_as_string());
					temp_player.shots = stoi(player->at(8)
						.get_value_as_string());
					temp_player.games = 1;
					if (players.count(temp_player.onlineID)) {
						players[temp_player.onlineID].score +=
							temp_player.score;
						players[temp_player.onlineID].goals +=
							temp_player.goals;
						players[temp_player.onlineID].assists +=
							temp_player.assists;
						players[temp_player.onlineID].saves +=
							temp_player.saves;
						players[temp_player.onlineID].shots +=
							temp_player.shots;
						players[temp_player.onlineID].games +=
							temp_player.games;
					} else {
	...
	output_file <<
		"Player Stats generated by RocketAnalyticsPlayerStatsParser.\n";
	output_file <<
		"Player #,Team,Name,Platform,Steam64,Games Played,Score,Goals,"
		"Assists,Saves,Shots\n";
	int count = 1;
